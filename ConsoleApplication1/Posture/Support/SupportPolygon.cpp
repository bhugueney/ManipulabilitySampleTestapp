...
struct SupportPolygonPImpl
{
	...
	SupportPolygonPImpl(const Robot& robot)
	{
		...
		{
			// IMPL no impl of islocked found, assumed false
			/*
			if ((*it)->IsLocked())
			{
	...
	double isLeft(const VectorX& P0, const VectorX& P1, const VectorX& P2)
	{
...
#include <iostream>
	...
	double DistancePointSegment(const Vector3& pt, const Vector2& A, const Vector2& B)
	{
		...
		else
		{
			for (unsigned int j = 1; j < points.size(); ++j)
			{
				...
				if (points.size() == 2 || intersection_.IntersectSegments(centroid, externalCom, A, B, interPoint))
				{
					...
					//line that goes through and orthogonal to segment
					double c = -(1 / a);
					double d = externalCom.y() - (externalCom.x() * c);
