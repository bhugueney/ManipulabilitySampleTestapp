...
void BuildWorld()
{
	...
	Vector3 p13(-0.8, 2 + decal, 1.8f);
	Vector3 p23( 3, -1 + decal, 1.8f);
...
int main(int argc, char *argv[])
{
	...
	const RobotI* pRobot = manager.CreateRobot(enums::robot::Spider, robotBasis);
	...
	robotBasis2(0) = -5;
	robotBasis2(2) = 0.2+ dec;
	postureManager->AddCheckPoint( 4, robotBasis2);

	robotBasis2(0) = -3;
	robotBasis2(2) = 0.4+ dec;
	postureManager->AddCheckPoint( 4, robotBasis2);

	robotBasis2(0) = -2;
	robotBasis2(2) = -0.1 + dec;
	...
	robotBasis2(0) = 12;
	robotBasis2(2) = 1.2 + dec;
	postureManager->AddCheckPoint( 9, robotBasis2);

	robotBasis2(0) = 18;
	robotBasis2(2) = 1.7 + dec;
...
/* trajectory */
	postureManager->ComputeThird(pRobot,10000);
/*TEST */
	#ifdef PROFILE
	postureManager->Log();
	#endif
