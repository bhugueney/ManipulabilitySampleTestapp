...
static float xyz[3] = {0.0,1,3.0};
static float hpr[3] = {70, 0.,0,};
RobotI* pRobot;

bool computePosture = true;

void PosePrise(const matrices::Vector3& p)
{
	Vector3 p1(p(0)+0.1, p(1)-0.2, p(2));
	Vector3 p11(p(0)+0.1, p(1)+0.1, p(2));
	Vector3 p12(p(0)-0.2, p(1)+0.1, p(2));
	Vector3 p2(p(0)-0.2, p(1)-0.2, p(2));
	manager.AddObstacle(p1, p11, p12, p2);
}
...
void XboxControlAnim(RobotI * robot)
{
	...
	if( xL != 0) // || yL != 0)
	{
		//double trans[3] = {((double)yL * slowDownFactor * 2),-((double)xL * slowDownFactor*2), 0.};
		double trans[3] = {((double)xL * slowDownFactor * 2),0, 0.};
		robot->Translate(trans);
		oldDir_ = Vector3(xL, 0, 0);
		oldDir_.normalize();
		transformed = true;
	}
	if( yR != 0)
	...
	{
		double trans[3] = {0, 0, -slowDownFactor*2};
		robot->Translate(trans);
	...
	if(transformed)
	{
		...
		//xyz[0] = transform(0,3) + 2;
		//xyz[1] = transform(1,3); // - 5.f;
		//xyz[2] = transform(2,3) + 10.f;
		/*
	...
	if(controller.AnyReleased((int)xbox::Buttons::X))
	{
