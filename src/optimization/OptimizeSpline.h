...
*/
	...
	*/
	...
	// adding boundaries for waypoint location relative to current waypoints
	// we have (size -2) * Dim constraints, (removing first and last).
	/*MatrixX waypts = MatrixX::Zero((size -2) * Dim, numvar);
	for(int i = 0; i<(size -2) * Dim; i = i + Dim)
	{
		for(int j = 0; j<Dim; ++j)
		{
			waypts(i, (i + 1) * Dim + j) = 1;
		}
	}*/
...
// NOG const MSKint32t numcon = 12 * size + 12;
	...
	//a.block(size * Dim * 2 + Dim * 4	, 0, (size -2) * Dim, numvar) = waypts;
	...
	*/
	...
	//We add bounds to intermediary waypoints variables to favorize convergence
	...
	In it(wayPointsBegin);
	++it;
	for(int i = Dim; i < (size -1) * Dim; i= i + Dim)
	{
		for(int j =0; j < Dim; ++j)
		{
			bkx[i+j] = MSK_BK_RA;
			blx[i+j] = it->second(j) - 10;
			bux[i+j] = it->second(j) + 10;
		}

		++it;
	}
	// other variables ( last waypoint + derivatives)
	for(int i = (size -1) * Dim; i < numvar; ++i)
	{
		bkx[i] = MSK_BK_FR;
		blx[i] = -MSK_INFINITY;
		bux[i] = +MSK_INFINITY;
	...
	if ( r==MSK_RES_OK )
	{
		...
		if ( r==MSK_RES_OK )
		{
			...
			if ( xx )
			{
				...
				Log(wayPointsBegin, wayPointsEnd, nwaypoints.begin(), nwaypoints.end());
				free(xx);
			}
		}
		else
